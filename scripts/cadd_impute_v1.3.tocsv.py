...
categorical = { "Type" : ["SNV", "INS", "DEL"],
                ...
                "Domain" : ["ncoils", "tmhmm", "sigp", "lcompl", "ndomain", "UD"],
...
domain_set = set(categorical["Domain"])
    ...
    if operation == None:
        ...
        if key in categorical:
            for category in categorical[key]:
                if key not in catvals:
                    catvals[key] = [(category, ind)]
                else:
                    catvals[key].append((category, ind))
...
for line in sys.stdin:
    ...
    for ind, (name, operation) in enumerate(transform):
        # print ind, name, operation, catvals, '\n'
        ...
        if name.startswith("IND_"):
            found = True
            if fields[ind] != "0": new_fields.append((indicators[name[4:]], "1"))
        else:
            cval = fields[ind]
            if name in intvals:
                found = True
                ...
                if float(cval) != 0: new_fields.append((intvals[name], cval))
    ...
    if fieldsDict["Consequence"] in products:
        for key2, pind in products[fieldsDict["Consequence"]].iteritems():
            ...
            if hval2 != None and float(hval2) != 0: 
                new_fields.append((pind, hval2))
    ...
    if options.portion == None:
        ...
        if random.random() < options.portion:
            training.write("%s,%s\n"%(state, ",".join(all_fields)))
        else:
            testing.write("%s,%s\n"%(state, ",".join(all_fields)))
